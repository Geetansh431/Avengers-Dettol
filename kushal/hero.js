let data_array = [
  ...
  },
  {
    hero: "Thor",
    name: "Thor Odinson",
    src: "images/thor-removebg-preview.png",
...
function display_info(tosearch) {
  data_array.forEach(function (value) {
    if (value.hero.toLowerCase() == tosearch.toLowerCase()) {
      ...
      let superpowerArray = value.superpowers.split(",");
      console.log(superpowerArray);
      superpowerArray.forEach(instr => {
        show_instructions += `<li>${instr.toUpperCase()}</li>`
      });
      cont.innerHTML =
        `<div class="left">
      <div class="image">
          <img  id="backgrd" src="./images/WhatsApp Image 2024-03-13 at 23.48.30_52b0e312.jpg" alt="">
          <img id="frontimage" src=${value.src} alt="">
      </div>
      <div class="bot">
      <p style="margin-bottom:5px; font-size:16px">${value.name}   --->    ${value.hero}</p>
          </div>
  </div>

  <div class="right">
      <div class="super">${show_instructions}
      </div>
...
searchherobtn.addEventListener("click", function () {
  ...
  if (tosearch == "") {
    display_info(data_array[1].hero);
    display_movie(data_array[1].hero);
  } else {
